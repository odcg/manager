...
export default class OrderController {
  ...
  getPricings() {
    this.pricing = null;
    this.planCode = this.eventHandlers.onBeforePricingGetPlancode();
  ...
  computePricing(catalogPricings) {
    ...
    if (this.type === ORDER_TYPES.RENEW) {
      ...
      if (
        ...
        (!pricingWithOnlyInstallationCapacity && pricingsWithOnlyRenewCapacity)
      ) {
  ...
  isFreePricing() {
    return get(this.pricing, 'price') === 0;
  }
  ...
  prepareCheckout() {
    ...
    return this.createNewCart()
      ...
      .catch((error) =>
        !this.eventHandlers.onError || this.eventHandlers.onError({ error })
  ...
  validateCheckout() {
    ...
    return this.WucOrderCartService.checkoutCart(
      ...
      .then((checkout) => {
        this.eventHandlers.onCheckoutSuccess({
          checkoutResult: {
            checkout,
